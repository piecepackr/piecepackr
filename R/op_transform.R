...
which_AABB_overlap <- function(dfi, dfs) {
	rev(which(
		do_ranges_overlap(dfi$xl, dfi$xr, dfs$xl, dfs$xr) &
...
compute_z <- function(df, zp, i, j, pt_thickness = 0.01) {
	if (all(df$piece_side[c(i, j)] == "pyramid_top")) {
		if (df$rank[i] <= df$rank[j]) {
			# top piece is (weakly) smaller
			zp[j] + 0.5 * df$depth[j] + pt_thickness - 0.5 * df$depth[i]
		} else {
			# top piece is (strictly) bigger
	...
	for (ii in seq(length.out = nrow(df))) {
		...
		if (
			...
			if (grepl("^concave", label)) {
				label <- gsub("concave", "convex", label)
			}
			shape <- pp_shape(
				label,
				opt$shape_t,
