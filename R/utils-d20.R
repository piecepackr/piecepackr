...
d20_xyz <- function(suit, rank, cfg, x, y, z, angle, axis_x, axis_y, width, height, depth) {
	R_rank <- d20_rot_to_top(rank)
	xs <- numeric(0)
	ys <- numeric(0)
	zs <- numeric(0)
	xyz_t <- as_coord3d(
		regular_ngon_polygon2d(3, x = 0, y = 0, theta = degrees(90)),
		z = 0.5
	)$scale(
		width,
		height,
	...
	as_coord3d(xs, ys, zs)$transform(AA_to_R(angle, axis_x, axis_y))$translate(as_coord3d(x, y, z))
}

save_d20_obj <- function(
	piece_side,
	suit,
	rank,
	cfg,
	...
	xy_vt <- list(
		x = rep(p$x / 4, 20) + rep(rep(0:3 / 4, each = 3), 5),
		y = rep(p$y / 5, 20) + rep(rep(4:0 / 5, each = 3), each = 4)
	)
