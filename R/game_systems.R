...
game_systems <- function(
	...
	list(
		alquerque = alquerque(1, color_list, shading),
		...
		dominoes_black = dominoes(
			color_list$suit_color[2],
			"white",
			color_list$border_color,
			rect_shape
		),
		dominoes_blue = dominoes(
			color_list$suit_color[4],
			"white",
			color_list$border_color,
			rect_shape
		),
		dominoes_green = dominoes(
			color_list$suit_color[3],
			"white",
			color_list$border_color,
			rect_shape
		),
		dominoes_red = dominoes(
			color_list$suit_color[1],
			"white",
			color_list$border_color,
			rect_shape
		),
		dominoes_white = dominoes(
			color_list$suit_color[6],
			"black",
			color_list$border_color,
			rect_shape
		),
		dominoes_yellow = dominoes(
			color_list$suit_color[5],
			"black",
			color_list$border_color,
			rect_shape
		),
...
go_stone <- function(cell_width, shading = FALSE) {
	list(
		width.bit = 0.8700787 * cell_width,
...
dominoes <- function(
	...
	dominoes <- pp_cfg(list(
		...
		background_color = background_color,
		mat_width = mat_width,
		mat_color = suit_color,
		border_color = border_color,
		border_lex = border_lex,
		die_arrangement = "opposites_sum_to_5",
		grob_fn.die = pippedGrobFn(0, "die"),
...
piecepack <- function(
	...
	if (font == "sans") {
		piecepack_suit_text <- list(suit_text = "\u263c,\u25d8,\u0238,\u03ee,\u2202", n_suits = 4L)
		pce_suit_text <- list(suit_text = "\u2665,\u2660,\u2663,\u2666,\u2202", n_suits = 4L)
		dpe_suit_text <- list(
			suit_text = "\u2665,\u2660,\u2663,\u2666,*,\u2202",
			suit_cex.s5 = 1.3,
			n_suits = 5L
		...
		piecepack_suit_text <- list(
			suit_text = "\u2742,\u25d0,\u265b,\u269c,\u0ed1",
			suit_cex.s2 = 0.9,
			dm_cex.coin = 0.5,
			n_suits = 4L
		)
		pce_suit_text <- list(suit_text = "\u2665,\u2660,\u2663,\u2666,\u0ed1", n_suits = 4L)
		dpe_suit_text <- list(
			suit_text = "\u2665,\u2660,\u2663,\u2666,\u2605,\u0ed1",
			suit_cex.s5 = 0.92,
			n_suits = 5L,
	...
	piecepack_base <- list(
		...
		suit_text.saucer_back = "",
		suit_cex.saucer_face = 0.9,
		suit_cex.pyramid_face = 0.5,
		ps_r.pyramid_face = -0.08,
		dm_cex.pyramid_face = 4.0,
		dm_text.pyramid_face = "|",
		dm_r.pyramid_face = -0.22,
		ps_cex.pyramid_left = 0.7,
		ps_r.pyramid_left = -0.00,
		ps_cex.pyramid_right = 0.7,
		ps_r.pyramid_right = -0.00,
	...
	dpe_base <- list(
		...
		border_color.s2.die = "black",
		border_color.s2.pawn = "black",
	...
	piecepack <- c(list(suit_color = cb_suit_colors_pure), piecepack)
...
playing_cards <- function(font = "sans", rect_shape = "rect") {
	if (font == "sans") {
		face_labels <- c("", "\u050a", "\u046a", "\u0238")
		fool_text <- "*"
		pc_suit_text <- list(suit_text = "\u2665,\u2660,\u2663,\u2666,*", suit_cex.s5 = 1.3)
	} else if (font == "dejavu") {
		face_labels <- c("", "\u265e", "\u265b", "\u265a")
		fool_text <- "\u2605"
		pc_suit_text <- list(suit_text = "\u2665,\u2660,\u2663,\u2666,\u2605", suit_cex.s5 = 0.92)
	}

	playing_cards_list <- list(
		n_ranks = 14,
	...
	playing_cards_colored$grob_fn.s5.r11.card_face <- faceCardGrobFn(
		face_labels[1],
		fill_stroke = TRUE
	)
	playing_cards_colored$grob_fn.s5.r12.card_face <- faceCardGrobFn(
		face_labels[3],
		fill_stroke = TRUE
	)
	playing_cards_colored$grob_fn.s5.r13.card_face <- faceCardGrobFn(
		face_labels[4],
		fill_stroke = TRUE
	)
