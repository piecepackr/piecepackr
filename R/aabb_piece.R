...
aabb_piece <- function(df,
    ...
    llb <- as_coord2d(coord3d(df$xll, df$yll, df$zb), alpha = op_angle, scale = op_scale)
    llt <- as_coord2d(coord3d(df$xll, df$yll, df$zt), alpha = op_angle, scale = op_scale)
    ulb <- as_coord2d(coord3d(df$xul, df$yul, df$zb), alpha = op_angle, scale = op_scale)
    ult <- as_coord2d(coord3d(df$xul, df$yul, df$zt), alpha = op_angle, scale = op_scale)
    lrb <- as_coord2d(coord3d(df$xlr, df$ylr, df$zb), alpha = op_angle, scale = op_scale)
    lrt <- as_coord2d(coord3d(df$xlr, df$ylr, df$zt), alpha = op_angle, scale = op_scale)
    urb <- as_coord2d(coord3d(df$xur, df$yur, df$zb), alpha = op_angle, scale = op_scale)
    urt <- as_coord2d(coord3d(df$xur, df$yur, df$zt), alpha = op_angle, scale = op_scale)
