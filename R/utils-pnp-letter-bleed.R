...
a5_inst_grob_bleed <- function(cfg, pieces, arrangement, size) {
	...
	# Coins
	coin_crop <- c(
		'\t\u25cb For those lacking circular cutting tools one may use "crop" marks',
	...
	# Dice and Pawns
	dice_crop <- c(
		'\t\u25cb Depending on size of target dice use inner or outer "crop" marks',
...
a5_pawn_grob <- function(suit, cfg, front, arrangement) {
	...
	if (!front) {
		df_pb <- tibble::tibble(piece_side = "pawn_back", x = x, suit = suit, y = ypb, angle = 180)
		df_pt <- tibble::tibble(piece_side = "pawn_face", x = x, suit = suit, y = ypt, angle = 0)
		...
		df_pb <- tibble::tibble(piece_side = "pawn_face", x = x, suit = suit, y = ypb, angle = 180)
		df_pt <- tibble::tibble(piece_side = "pawn_back", x = x, suit = suit, y = ypt, angle = 0)
