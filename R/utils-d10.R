...
d10TopGrob <- function(
	...
	gl <- gList()
	fn_idx <- function(idx) {
		switch(idx, 1:4, 5:8, 9:12, 13:16, 17:20, 21:24, 25:28, 29:32, 33:36, 37:40)
	}
	...
	gTree(
		...
		children = gl,
		cl = c("projected_rpg_die", "coords_xyl")
	)
}
...
d10_xyz <- function(suit, rank, cfg, x, y, z, angle, axis_x, axis_y, width, height, depth) {
	opt <- cfg$get_piece_opt("die_face", suit, rank)
	xy <- kite_xy(r = 0.5 - 0.1909830056250526320039)
	if (nigh(opt$shape_t, 90) || nigh(opt$shape_t, 270)) {
		xyz_t <- as_coord3d(xy$x, xy$y, 0.0)$translate(as_coord3d(-0.5, -0.5, 0.5))$scale(
			width,
			height,
			depth
		)
		xyz_b <- as_coord3d(xy$x, xy$y, 0.0)$translate(as_coord3d(-0.5, -0.5, -0.5))$rotate(
			"z-axis",
			degrees(180)
		)$scale(width, height, depth)
	} else {
		xyz_t <- as_coord3d(xy$x, xy$y, z = 0.0)$translate(as_coord3d(-0.5, -0.5, 0.5))$scale(
			height,
			width,
			depth
		)
		xyz_b <- as_coord3d(xy$x, xy$y, z = 0.0)$translate(as_coord3d(-0.5, -0.5, -0.5))$rotate(
			"z-axis",
			degrees(180)
...
kite_xyz_missing_right <- function(p_top, p_left, p_bottom) {
	...
	as_coord3d(
		x = c(p_top$x, p_left$x, p_bottom$x, p_right$x),
...
save_d10_obj <- function(
	...
	xy <- kite_xy(r = 0.5 - 0.1909830056250526320039)
	xy_vt <- list(
		x = rep(xy$x / 2, 10) + rep(rep(0:1 / 2, each = 4), 5),
	...
	f[[1]] <- list(v = 1:4, vt = fn_vt(rank))
	f[[2]] <- list(v = 5:8, vt = fn_vt(d10_edge_rank("d10_left1", rank)))
	f[[3]] <- list(v = 9:12, vt = fn_vt(d10_edge_rank("d10_left2", rank)))
	f[[4]] <- list(v = 13:16, vt = fn_vt(d10_edge_rank("d10_right1", rank)))
	f[[5]] <- list(v = 17:20, vt = fn_vt(d10_edge_rank("d10_right2", rank)))
	f[[6]] <- list(v = 21:24, vt = fn_vt(d10_edge_rank("d10_opposite", rank)))
	f[[7]] <- list(v = 25:28, vt = fn_vt(d10_edge_rank("d10_opposite_left1", rank)))
	f[[8]] <- list(v = 29:32, vt = fn_vt(d10_edge_rank("d10_opposite_left2", rank)))
	f[[9]] <- list(v = 33:36, vt = fn_vt(d10_edge_rank("d10_opposite_right1", rank)))
	f[[10]] <- list(v = 37:40, vt = fn_vt(d10_edge_rank("d10_opposite_right2", rank)))
