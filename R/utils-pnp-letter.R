...
print_and_play_paper <- function(cfg, size, pieces, arrangement, quietly) {
    n_suits <- cfg$n_suits
    n_ranks <- cfg$n_ranks
...
a5_title_grob <- function(cfg, pieces, quietly, extra_credit=TRUE) {
    ...
    if (is.null(cfg$title)) {
        if (!quietly) message("`cfg$title` is `NULL`, omitting title")
        grob_title <- nullGrob()
    } else {
    ...
    if (is.null(cfg$description)) {
        if (!quietly) message("`cfg$description` is `NULL`, omitting description")
    ...
    if (is.null(cfg$spdx_id)) {
        if (!quietly) message("`cfg$spdx_id` is `NULL`, omitting license")
    ...
    if (is.null(cfg$copyright) && !quietly)
        if (!quietly) message("`cfg$copyright` is `NULL`, omitting copyright")
    ...
    if (is.null(cfg$credit) && !quietly)
        message("`cfg$credit` is `NULL`, omitting custom credits")
    if (extra_credit) {
        credits <- paste(c("\u25cf This print-and-play layout was generated by piecepackr.",
