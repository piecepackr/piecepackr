...
#'   lookat <- c(0, 0, 0)
#'   try(rayvertex::rasterize_scene(mesh, light_info = light, lookat = lookat))
#'   invisible(options(opt))
#' }
#' supports_piece_mesh <- requireNamespace("rayvertex", quietly = TRUE) &&
#'   all(capabilities(c("cairo", "png")))
#' if (supports_piece_mesh) {
...
#' }
#' @export
#' @seealso See \url{https://www.rayvertex.com} for more information about the `rayvertex` package.
