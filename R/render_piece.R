...
render_piece <- function(
	...
	if (is.null(width)) {
		width <- dfr$x_op[2] + xoffset + 0.50
	}
	if (is.null(height)) {
		height <- dfr$y_op[2] + yoffset + 0.50
	}
	if (is.na(width)) {
		width <- 1
	}
	...
	if (open_device) {
		...
		args <- c(list(filename = file, width = width, height = height), dev.args)
...
plot_fn_helper <- function(
	...
	if (identical(.f, grid.piece)) {
		...
		function(
			...
			if (all(is.na(table))) {
				table <- rayrender::sphere(
					z = -1e3,
					radius = 1e3,
					material = rayrender::diffuse(color = "green")
				)
				light <- rayrender::sphere(
					x = 0.5 * width / ppi,
					y = -4,
					z = max(2.0 * m + 1, 20),
			...
			if (is.null(lookfrom)) {
				lookfrom <- c(0.5 * width / ppi, -2.0 * m, 2.0 * m)
			}
		...
		function(
			...
			if (all(is.na(light_info))) {
				light_info <- rayvertex::directional_light(
					c(0.5 * width / ppi, -4, max(2.0 * m + 1, 20)),
					intensity = 2.5
				)
			}
...
annotate_plot <- function(
	...
	if (is.null(xbreaks)) {
		...
		x_coords <- seq(annotation_scale, by = annotation_scale, length.out = max(xbreaks))
	}
	...
	if (is.null(ybreaks)) {
		...
		y_coords <- seq(annotation_scale, by = annotation_scale, length.out = max(ybreaks))
	}
