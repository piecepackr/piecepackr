...
crosshairGrob <- function(
	...
	if (!is.unit(ch_width)) {
		ch_width <- unit(ch_width, default.units)
	}
...
squaresCrosshairGrob <- function(
	...
	for (i in seq(nn)) {
		...
		gl[[i]] <- grobTree(
			rectGrob(
				x = c(0.1, 0.3, 0.5, 0.5, 0.5, 0.5, 0.5, 0.7, 0.9),
			...
			rectGrob(
				x = c(0.2, 0.4, 0.5, 0.5, 0.5, 0.5, 0.6, 0.8),
