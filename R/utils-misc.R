...
file2grob <- function(file, distort = TRUE) {
	...
	if (inherits(picture, "Picture")) {
		...
		# grDevices::is.raster(picture) # nolint
...
device_supports_unicode <- function() {
	...
	if (
		any(vapply(
			unicode_devices,
			function(x) grepl(paste0("^", x), device),
...
at_inform <- function(fallback = "picture") {
	...
	if (fallback == "picture") {
		msg <- paste(
			msg,
			"Falling back to rendering piece side with `grImport2::pictureGrob(..., distort=TRUE)`."
		)
	} else {
...
fs_inform <- function() {
	...
	msg <- paste(
		"Stroking and filling path support not detected in the active graphics device.",
