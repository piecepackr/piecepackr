...
get_decks <- function(cfg=list()) {
    ...
    split(make_get_style_fn("decks", default)(cfg))
}
...
make_collection_preview <- function(cfg) {
    dir.create(get_pdf_preview_dir(cfg), recursive=TRUE, showWarnings=FALSE)

    decks <- get_decks(cfg)
    fp <- file.path(get_pdf_preview_dir(cfg), paste0(cfg$filename, ".pdf"))
    pp_pdf(fp, get_font(cfg), get_paper(cfg))
    ...
    for (ii in 1:n_pages) {
        ...
        for(kk in 0:5) {
            ...
            if(is.na(deck))
                ...
                l_logos[[kk+1]] <- pictureGrob(readPicture(file.path(get_svg_preview_dir(cfg), paste0(deck, ".svg")), warn=FALSE))
        }
...
make_pnp_piecepack <- function(cfg) {
    directory <- get_pdf_deck_dir(cfg)
    dir.create(directory, recursive=TRUE, showWarnings=FALSE)

    pdf_file <- file.path(directory, paste0(get_deck_filename(cfg), ".pdf"))
    unlink(pdf_file)

    pp_pdf(pdf_file, get_font(cfg), get_paper(cfg))
...
make_pdfmark_txt <- function(pm_filename, cfg) {
    deck_filenames <- file.path(get_pdf_deck_dir(cfg), paste0(get_decks(cfg), ".pdf"))
    n_sets <- length(deck_filenames)
    txt <- paste0("[ /Title (", get_title(cfg), ")\n /Author (", get_author(cfg), ")\n /Subject (", 
...
make_collection <- function(cfg) {
    dir.create(get_pdf_collection_dir(cfg), recursive=TRUE, showWarnings=FALSE)

    deck_filenames <- shQuote(file.path(get_pdf_deck_dir(cfg), paste0(get_decks(cfg), ".pdf")))
    fp <- shQuote(file.path(get_pdf_preview_dir(cfg), paste0(cfg$filename, ".pdf")))
    # files <- paste(fp, paste(shQuote(deck_filenames), collapse=" "))

    # add bookmarks
    pm_filename <- tempfile(fileext=".txt")
    make_pdfmark_txt(pm_filename, cfg)
    bf <- shQuote(file.path(get_pdf_collection_dir(cfg), paste0(cfg$filename, ".pdf")))
    cmd <- find_gs()
