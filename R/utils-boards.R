...
checkeredBoardGrobFn <- function(nrows = 8, ncols = 8, margin = 0) {
	# nolint
	force(nrows)
	force(ncols)
	force(margin)
...
makeContent.checkered_board <- function(x) {
	...
	vp_cell <- viewport(
		width = x$ncols / (x$ncols + 2 * x$margin),
		height = x$nrows / (x$nrows + 2 * x$margin)
	)
	cell_grob <- cellsGrob(
		x$nrows,
		x$ncols,
		gp = gpar(fill = opt$gridline_color),
		name = "cells",
		vp = vp_cell
	)
...
linedBoardGrobFn <- function(nrows = 8L, ncols = 8L, margin = 0) {
	# nolint
	force(nrows)
	force(ncols)
	force(margin)
...
holedBoardGrobFn <- function(nrows = 4L, ncols = 4L, margin = 0) {
	# nolint
	force(nrows)
	force(ncols)
	force(margin)
...
npc2snpc <- function(x) {
	vapply(
		...
		function(x) {
			max(
				convertX(unit(x, "npc"), "npc", valueOnly = TRUE),
...
alquerqueInnerGrob <- function(gp = gpar(), name = NULL, diag = TRUE) {
	...
	tl_br1 <- polygonGrob(
		x = c(1 - wd, 1 + wd, 1 + wd, 0 + wd, 0 - wd, 0 - wd, 1 - wd),
		y = c(0 - wd, 0 - wd, 0 + wd, 1 + wd, 1 + wd, 1 - wd, 0 - wd)
	)
	tl_br2 <- polygonGrob(
		x = c(1 / 2 - wd, 1 / 2 + wd, 1 / 2 + wd, 0 + wd, 0 - wd, 0 - wd, 1 / 2 - wd),
		y = c(0 - wd, 0 - wd, 0 + wd, 1 / 2 + wd, 1 / 2 + wd, 1 / 2 - wd, 0 - wd)
	)
	tl_br3 <- polygonGrob(
		x = c(1 - wd, 1 + wd, 1 + wd, 1 / 2 + wd, 1 / 2 - wd, 1 / 2 - wd, 1 - wd),
		y = c(1 / 2 - wd, 1 / 2 - wd, 1 / 2 + wd, 1 + wd, 1 + wd, 1 - wd, 1 / 2 - wd)
	)
	tr_bl1 <- polygonGrob(
		x = c(0 + wd, 0 - wd, 0 - wd, 1 - wd, 1 + wd, 1 + wd, 0 + wd),
		y = c(0 - wd, 0 - wd, 0 + wd, 1 + wd, 1 + wd, 1 - wd, 0 - wd)
	)
	tr_bl2 <- polygonGrob(
		x = c(0 + wd, 0 - wd, 0 - wd, 1 / 2 - wd, 1 / 2 + wd, 1 / 2 + wd, 0 + wd),
		y = c(1 / 2 - wd, 1 / 2 - wd, 1 / 2 + wd, 1 + wd, 1 + wd, 1 - wd, 1 / 2 - wd)
	)
	tr_bl3 <- polygonGrob(
		x = c(1 / 2 + wd, 1 / 2 - wd, 1 / 2 - wd, 1 - wd, 1 + wd, 1 + wd, 1 / 2 + wd),
		y = c(0 - wd, 0 - wd, 0 + wd, 1 / 2 + wd, 1 / 2 + wd, 1 / 2 - wd, 0 - wd)
	)
...
morrisBoard6Grob <- function(gp = gpar(), name = NULL, cross = FALSE) {
	...
	if (cross) {
		dots <- circleGrob(
			x = rep.int(c(0, 1, 2, 3, 4), c(3, 3, 5, 3, 3)) / 4,
		...
		dots <- circleGrob(
			x = rep.int(c(0, 1, 2, 3, 4), c(3, 3, 4, 3, 3)) / 4,
...
morrisBoard9Grob <- function(gp = gpar(), name = NULL, diag = FALSE) {
	...
	wd <- w / sqrt(2) / 2
	vlines <- rectGrob(
		x = c(0, 1, 2, 3, 3, 4, 5, 6) / 6,
		y = c(3, 3, 3, 1, 5, 3, 3, 3) / 6,
		width = w,
		height = c(6, 4, 2, 2, 2, 2, 4, 6) / 6 + w
	)
	hlines <- rectGrob(
		x = c(3, 3, 3, 1, 5, 3, 3, 3) / 6,
		y = c(0, 1, 2, 3, 3, 4, 5, 6) / 6,
		height = w,
		width = c(6, 4, 2, 2, 2, 2, 4, 6) / 6 + w
	)
	dots <- circleGrob(
		x = rep.int(c(0, 1, 2, 3, 4, 5, 6), c(3, 3, 3, 6, 3, 3, 3)) / 6,
		y = c(0, 3, 6, 1, 3, 5, 2, 3, 4, 0, 1, 2, 4, 5, 6, 2, 3, 4, 1, 3, 5, 0, 3, 6) / 6,
		r = 1.5 * w
	)
	if (diag) {
		ul <- polygonGrob(
			x = c(2 / 6 - wd, 2 / 6 + wd, 2 / 6 + wd, 0 + wd, 0 - wd, 0 - wd, 2 / 6 - wd),
			y = c(0 - wd, 0 - wd, 0 + wd, 2 / 6 + wd, 2 / 6 + wd, 2 / 6 - wd, 0 - wd) + 4 / 6
		)
		br <- polygonGrob(
			x = c(2 / 6 - wd, 2 / 6 + wd, 2 / 6 + wd, 0 + wd, 0 - wd, 0 - wd, 2 / 6 - wd) + 4 / 6,
			y = c(0 - wd, 0 - wd, 0 + wd, 2 / 6 + wd, 2 / 6 + wd, 2 / 6 - wd, 0 - wd)
		)
		ur <- polygonGrob(
			x = c(0 + wd, 0 - wd, 0 - wd, 2 / 6 - wd, 2 / 6 + wd, 2 / 6 + wd, 0 + wd) + 4 / 6,
			y = c(0 - wd, 0 - wd, 0 + wd, 2 / 6 + wd, 2 / 6 + wd, 2 / 6 - wd, 0 - wd) + 4 / 6
		)
		bl <- polygonGrob(
			x = c(0 + wd, 0 - wd, 0 - wd, 2 / 6 - wd, 2 / 6 + wd, 2 / 6 + wd, 0 + wd),
			y = c(0 - wd, 0 - wd, 0 + wd, 2 / 6 + wd, 2 / 6 + wd, 2 / 6 - wd, 0 - wd)
		)
	} else {
		ul <- NULL
