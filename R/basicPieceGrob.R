...
makeContent.basic_piece_side <- function(x) {
	...
	gl_grob <- shape$gridlines(
		gp = gpar(col = opt$gridline_color, lex = opt$gridline_lex),
		name = "gridlines",
		mat_width = bg_mat_width
	)
...
pyramidTopGrob <- function(piece_side, suit, rank, cfg = pp_cfg()) {
	gTree(suit = suit, rank = rank, cfg = cfg, scale = 1, cl = "pyramid_top")
}
...
previewLayoutGrob <- function(piece_side, suit, rank, cfg = pp_cfg()) {
	...
	gl[["saucers"]] <- pieceGrob(
		c("saucer_face", "saucer_back"),
		c(1, NA),
		NA,
		cfg,
		t_width + 1.5 * d_width,
		0.5 * t_width + c(0.5, -0.5) * s_width,
		default.units = "in",
		name = "saucers"
	)

	gl[["pawns"]] <- pieceGrob(
		c("pawn_face", "pawn_back"),
		2,
		NA,
		cfg,
		t_width + 3 * d_width,
		0.5 * t_width + c(0.5, -0.5) * p_height,
		angle = c(0, 180),
		default.units = "in",
		name = "pawns"
	)

	gl[["suitrankdie"]] <- pieceGrob(
		"suitrankdie_layoutRF",
		NA,
		NA,
		cfg,
		preview_width - 2 * d_width,
		0.5 * t_width,
		default.units = "in",
		name = "suitrankdie"
	)
...
piecepackDieGrob <- function(suit, cfg, flip = FALSE, arrangement = cfg$die_arrangement) {
	cfg <- as_pp_cfg(cfg)
...
suitdieGrob <- function(cfg, flip = FALSE) {
	...
	for (ii in 1:6) {
		...
		gl[[ii]] <- pieceGrob("suitdie_face", suit[ii], NA, cfg, default.units = "npc", vp = vp)
	}
