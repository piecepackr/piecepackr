...
save_print_and_play <- function(
	...
	dev = NULL,
	dev.args = list(family = cfg$fontfamily, onefile = TRUE, units = "in", bg = "white", res = 300),
	quietly = FALSE,
	...,
	bleed = FALSE,
	size_bleed = NULL
	...
	stopifnot(
		all(c("top", "right", "bottom", "left") %in% names(size_bleed)),
	...
	height <- switch(size, letter = LETTER_WIDTH, A4 = A4_WIDTH, A5 = A4_WIDTH, `4x6` = 4) +
		size_bleed$top +
		size_bleed$bottom
	width <- switch(size, letter = LETTER_HEIGHT, A4 = A4_HEIGHT, A5 = A4_HEIGHT / 2, `4x6` = 6) +
		size_bleed$right +
		size_bleed$left
	...
	if (!onefile && !has_c_integer_format(output_filename)) {
		abort(paste(
			"`save_print_and_play()` generates multiple pages.",
...
add_pdf_metadata <- function(output_filename, cfg = pp_cfg(), pl = list()) {
	...
	if (xmpdf::supports_set_docinfo()) {
		...
		msg <- c(
			x = "Unable to embed pdf documentation info metadata",
	...
	if (xmpdf::supports_set_bookmarks()) {
		...
		bookmarks <- data.frame(title = names(pl), page = starting_pages, stringsAsFactors = FALSE)
		xmpdf::set_bookmarks(bookmarks, output_filename)
	} else if (!isFALSE(getOption("piecepackr.metadata.inform"))) {
		msg <- c(
			x = "Unable to embed pdf bookmarks metadata",
