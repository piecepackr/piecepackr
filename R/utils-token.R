Token2S <- R6Class(
	...
	public = list(
		initialize = function(
			...
			# coordinates
			xyz_scaled <- as_coord3d(shape$npc_coords)$translate(as_coord3d(-0.5, -0.5, 0.5))$scale(
				whd$width,
				whd$height,
				whd$depth
			)
			...
			xyz_scaled <- as_coord3d(shape$npc_coords)$translate(as_coord3d(
				-0.5,
				-0.5,
				-0.5
			...
			xyz_scaled <- as_coord3d(xy_vp)$translate(as_coord3d(-0.5, -0.5, 0.5))$scale(
				whd$width,
				whd$height,
				whd$depth
			)
		...
		op_edge_order = function(angle) {
			...
			op_diff <- as_coord3d(degrees(180 + angle), radius = r, z = 0)
			op_ref <- mean(self$xyz)$translate(op_diff)
		...
		visible_side = function(angle) {
			...
			op_plane <- as_plane3d(normal = op_diff, p1 = op_ref)
			if (
				distance3d(op_plane, mean(self$xyz_face)) <
...
partition_edges <- function(shape) {
	...
	if (all(classes == "C0")) {
		...
		if (!(classes[n] == "C0")) {
			abort("Can only handle case when last class is 'C0'")
		}
...
Edge <- R6Class(
	...
	public = list(
		...
		visible_side = function(angle) {
			...
			op_plane <- as_plane3d(normal = op_diff, p1 = op_ref)
			if (
				distance3d(op_plane, mean(self$vertices_face)) <
		...
		vertices_visible_side = function(angle) {
			...
			switch(side, face = self$vertices_face, back = self$vertices_back)
		}
	),
	private = list(),
	active = list(
...
RingEdge <- R6Class(
	...
	private = list(
		op_grob_nonperpendicular = function(angle, scale, name = NULL) {
			...
			xy_visible <- as_coord2d(
				self$vertices_visible_side(angle),
				alpha = degrees(angle),
				scale = scale
			)
